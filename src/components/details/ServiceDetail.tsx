...
export default function ServiceDetail({ id, open, onOpenChange }: { id: string | null; open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const res: any = await api.services.get(id as string);
        if (!mounted) return;
        ...
        if (!payload) console.debug('ServiceDetail: response payload empty', res);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <Card>
          <CardHeader>
            <CardTitle>{item ? item.nom : 'Service'}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {loading && <div>Chargement...</div>}
              {!loading && !item && <div>Aucun service trouvé</div>}
              {!loading && item && (
