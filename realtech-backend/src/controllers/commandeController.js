...
/**
 * Delete commande (soft delete) and restore stock
 */
export const deleteCommande = asyncHandler(async (req, res) => {
  ...
  if (commande.statut === 'LIVREE') {
    throw new ApiError(400, 'Impossible de supprimer une commande livrée');
  }

  // Restore product stock for commande items that reference products
  if (Array.isArray(commande.items) && commande.items.length > 0) {
    for (const it of commande.items) {
      if (it.produit_id) {
        await ProductModel.updateStock(Number(it.produit_id), Number(it.quantite), 'ADD');
      }
    }
  }

  // Soft delete commande
  await CommandeModel.softDeleteCommande(Number(id));
    ...
    setImmediate(async () => {
      try {
        ...
        try {
          ...
          await NotificationModel.default.createNotification({ titre, message: msg, meta: { commandeId: freshCmd.id, montant }, lu: false });
          // if partial payment, notify about remaining
          if (remaining > 0) {
            await NotificationModel.default.createNotification({ titre: 'Paiement partiel', message: `Il reste ${remaining} à payer pour la commande ${freshCmd.numero || freshCmd.code}`, meta: { commandeId: freshCmd.id, reste: remaining }, lu: false });
          }
        } catch (eNotif) {
        ...
        try {
          ...
          if (!vente && paiementRecord) {
            try {
              ...
              vente = await VenteModel.createVente({
                code: venteCode,
                numero: venteNumero,
                montant: Number(paiementRecord.montant || added || 0),
          ...
          const receiptData = {
            ...
            commande: { numero: freshCmd.numero, client: { prenom: freshCmd.client_prenom, nom: freshCmd.client_nom }, utilisateur: { id: req.user?.userId || null } }
          };
